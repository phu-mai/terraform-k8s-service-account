...
variable "num_rbac_cluster_roles" {
  description = "Number of RBAC cluster roles to bind. This should match the number of items in the list passed to rbac_cluster_roles."
  type        = number
  default     = 0
}

variable "rbac_cluster_roles" {
  description = "List of maps representing RBAC cluster roles that should be bound to the service account. If this list is non-empty, you must also pass in num_cluster_rbac_roles specifying the number of cluster roles. This expects a list of maps, each with keys name and namespace."
