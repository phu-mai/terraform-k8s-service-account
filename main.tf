...
resource "kubernetes_cluster_role_binding" "service_account_cluster_role_binding" {
  count = var.create_resources ? var.num_rbac_cluster_roles : 0

  metadata {
    name = "${var.name}-${var.rbac_cluster_roles[count.index]["name"]}-cluster-role-binding"
  }

  role_ref {
    api_group = "rbac.authorization.k8s.io"
    kind      = "ClusterRole"
    name      = var.rbac_cluster_roles[count.index]["name"]
  }
